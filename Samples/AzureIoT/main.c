...
static void UartEventHandler(EventLoop *el, int iUARTFd, EventLoop_IoEvents events, void *context);
static void SendUartMessage(int iUARTFd, const uint8_t *pu8DataToSend, int iDataSize);
...
/// </summary>
/// <param name="iUARTFd">The open file descriptor of the UART to write to</param>
/// <param name="pu8DataToSend">The data to send over the UART</param>
/// <param name="iDataSize">The count of data to send over the UART</param>
static void SendUartMessage(int iUARTFd, const uint8_t *pu8DataToSend, int iDataSize)
{
    size_t totalBytesSent = 0;
    size_t totalBytesToSend = (size_t)iDataSize;
    ...
    while (totalBytesSent < totalBytesToSend) {
        ...
        const uint8_t *remainingMessageToSend = pu8DataToSend + totalBytesSent;
        ssize_t bytesSent = write(iUARTFd, remainingMessageToSend, bytesLeftToSend);
        if (bytesSent == -1) {
...
static void AzureTimerEventHandler(EventLoopTimer *timer)
{
    ...
    #define RS485_REQUEST_LEN 8
    rs485ReadCnt++;
    if (rs485ReadCnt == AzureIoTPollPeriodsPerTelemetry) {
        rs485ReadCnt = 0;
        // Read RS-485 device telemetry
        static char request485[RS485_REQUEST_LEN] = {0x01, 0x04, 0x00, 0x00,
                                                     0x00, 0x0A, 0x70, 0x0D};
        SendUartMessage(g_iUARTFd, request485, RS485_REQUEST_LEN);
    }
}
...
static void UartEventHandler(EventLoop *el, int iUARTFd, EventLoop_IoEvents events, void *context)
{
    ...
    ssize_t bytesRead;
    static char sendbuf[TELEMETRY_BUFFER_SIZE];
    int voltage, current, power_factor, frequency;
    int len;

    // Read incoming UART data. It is expected behavior that messages may be received in multiple
    // partial chunks.
    // bytesRead = read(s_iUARTFd, receiveBuffer, receiveBufferSize);
    bytesRead = read(iUARTFd, receiveBuffer, receiveBufferSize);
    if (bytesRead == -1) {
    ...
    if (bytesRead > 0) {
        ...
        Log_Debug("UART received %d bytes : ", bytesRead);
        for (int i = 0; i < bytesRead; i++) {
            Log_Debug("0x%02X ", receiveBuffer[i]);
        }
        Log_Debug("\n");
        if (bytesRead >= 12) {
            if (receiveBuffer[0] == 0x01 && receiveBuffer[1] == 0x04 && receiveBuffer[2] == 0x14) {
                voltage = (receiveBuffer[3] << 8 | receiveBuffer[4]) / 10;
                len = snprintf(sendbuf, TELEMETRY_BUFFER_SIZE, "{\"MeasuredVoltage\":%d}", voltage);
                SendTelemetry(sendbuf);
                Log_Debug("measured voltage : %d V\n", voltage);
                current = receiveBuffer[5] << 8 | receiveBuffer[6];
                len = snprintf(sendbuf, TELEMETRY_BUFFER_SIZE, "{\"MeasuredCurrent\":%d}", current);
                SendTelemetry(sendbuf);
                Log_Debug("measured current : %d mA\n", current);
            } else {
                frequency = (receiveBuffer[5] << 8 | receiveBuffer[6]) / 10;
                len = snprintf(sendbuf, TELEMETRY_BUFFER_SIZE, "{\"LineFrequecy\":%d}", frequency);
                SendTelemetry(sendbuf);
                Log_Debug("power line frequency : %d Hz\n", frequency);
                power_factor = receiveBuffer[7] << 8 | receiveBuffer[8];
                len = snprintf(sendbuf, TELEMETRY_BUFFER_SIZE, "{\"PowerFactor\":%d}", power_factor);
                SendTelemetry(sendbuf);
                Log_Debug("power factor : %d %%\n", power_factor);
            }
        }
    }
}
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    // open UART for RS-485 data transmit
    // Create a UART_Config object, open the UART and set up UART event handler
    UART_Config uartConfig;
    UART_InitConfig(&uartConfig);
    uartConfig.baudRate = 9600;
    uartConfig.flowControl = UART_FlowControl_None;
    g_iUARTFd = UART_Open(SAMPLE_UART_RS485, &uartConfig);
    if (g_iUARTFd == -1) {
        Log_Debug("ERROR: Could not open UART: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_UartOpen;
    }
    uartEventReg =
        EventLoop_RegisterIo(eventLoop, g_iUARTFd, EventLoop_Input, UartEventHandler, NULL);
    ...
    azureIoTPollPeriodSeconds = AzureIoTPollPeriodsPerTelemetry; //AzureIoTDefaultPollPeriodSeconds;
...
void SendSimulatedTelemetry(void)
{
    ...
    static float temperature = 25.0f;                    // starting temperature
